...
def existe_tarea(prior: str, hilo: str, tarea: str) -> bool:
    ...
    if not tareas_hilo: return False
    filtrados = list(map(parse_tarea,map(lambda x: TAREAS.get(x), tareas_hilo)))
    ids_posibles = list(filter(lambda x: parse_tarea(TAREAS.get(x)) in filtrados, tareas_hilo))
...
def modificar(id: int, tarea: str, add_priority: str = "", add_thread: str = "") -> None:
    ...
    prior = add_priority if (add_priority and add_priority != tarea_format[0]) else tarea_format[0]
    thread = add_thread if (add_thread and add_thread != tarea_format[1]) else tarea_format[1]
    tarea_f = "{}\n".format(tarea) if (tarea != tarea_format[2]) else tarea_format[2]
...
def print_by_prior(tareas_id: List[int]) -> None:#List[Dict[str, Union[Tuple[str, str, str],int]]]) -> None:
...
def print_hilo(hilo_name: str) -> None:
    ...
    if not idL:
        print_with_color("No existe el hilo \"{}\" putilla!".format(hilo_name), COLORES.get("ALTA"))
        return
    header = COLORES.get("HEADER")
    reset = COLORES.get("RESET")
    print_with_color("{}\n|{}|\n{}\n".format("-"*15,hilo_name,"-"*15), header)
    # tareas = [ {'id': x, 'tarea':parse_tarea(TAREAS.get(x))} for x in idL]  # type: List[Dict[str, Union[Tuple[str, str, str],int]]]
    print_by_prior(idL)
    print()
...
def remove_tarea(id: int) -> None:
    ...
    if not id in TAREAS:
        print_with_color("No hay tarea con id {}".format(id), COLORES.get("ALTA"))
        return
    clean_file()
...
def main(add_tarea: str, add_priority: str, add_thread: str,
    ...
    if not any([add_tarea, add_priority, add_thread, show_thread, show_thread_names, remove_id, modify_tarea]):
        ...
        print_with_color("ERROR: -t and -p go with -a or -s", COLORES.get("ALTA"))
        # exit(1)
    elif add_tarea:
