...
function potencia_sp(M::SparseMatrixCSC{Float64,Int64},
                     tolerancia::Float64,
                     n_iter::Int64=500,
                     r::Array{Float64,1}=ones(size(M,1)))
    i = 0
    n = size(M)[1]
...
function stationary_distribution(M::SparseMatrixCSC{Float32,Int32},
                                 tolerancia::Float64=1e-12,
                                 n::Int32=convert(Int32,size(M,1)),
                                 alfa::Float64=0.85,
                                 n_iter::Int64=500,
                                 order::Int32=convert(Int32,n),
                                 r::Vector{Float64} = zeros(n),
                                 last::Vector{Float64} = zeros(n))
    #Start
    r[1] = last[1]+1 # to ensure the first iterationon

    absorving = calculate_absorving(M,n)
    i = 0
